...
$output = '<div class="' . $css_class .' '.$bgcolor. '">';
...
foreach ( $graph_lines_data as $line ) {
    ...
    $padding_right=100-$percentage_value;
    if(isset( $customcolor)){
    	$bg_percentage="color:".$customcolor.";";
    }else{
    	$bg_percentage="";
    }
    $percentage_style="style='float:right; padding-right:".$padding_right."%; ".$bg_percentage.";'";
	$unit = ( $units != '' ) ? ' <span class="vc_label_units">' . $line['value'] . $units . '</span>' : '';
	$output .= '<small class="vc_label">' . $line['label'] .'<span class="vc_percentage " '.$percentage_style.' >'.$percentage_value.'%</span></small>';
	$output .= '<div class="vc_single_bar' . $bgcolor . '">';
	$output .= '<span class="vc_bar' . $bar_options . '" data-percentage-value="' . ( $percentage_value ) . '" data-value="' . $line['value'] . '" ' ;
